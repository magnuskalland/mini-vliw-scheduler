...
//        System.out.printf("Attempting to insert %d%s at %d\n", instruction.getAddress(), instruction, index);
    ...
    private void insertPrepareInstructions() {
        MovLoop movLoop = new MovLoop(getLoopStartAddress()-1, "EC", 1);
    ...
    public void collapseLoop() {
        bundles.subList(getLoopStartAddress() + initiationInterval, getLoopEndAddress())
        ...
        bundles.subList(getLoopStartAddress() + initiationInterval, getLoopEndAddress()).clear();
    ...
    private void addPredicates() {
        for (int i = getLoopStartAddress(); i < getLoopEndAddress(); i += initiationInterval) {
...
//                System.out.printf("%d%s made %d%s be scheduled in wrong bundle\n", p.getAddress(), p, c.getAddress(), c);
    ...
    private void mapRemainingRegisters() {
        ...
        resolveBasicBlockLocalDependencies();
        resolvePostLoopDependencies();
        resolveBasicBlockInvariants(new ArrayList<>(bundles.subList(0, getLoopStartAddress())));
        resolveBasicBlockInvariants(new ArrayList<>(bundles.subList(getLoopEndAddress(), bundles.size())));
...
//                        System.out.printf("\tSetting mapped register to x%d(%d, %d)\n", opt.get().getMappedDestination(), 1, -loopStage);
    ...
    private void resolveBasicBlockLocalDependencies() {
        // resolve local dependencies
        mapConsumed(new ArrayList<>(bundles.subList(0, getLoopStartAddress())));
        mapConsumed(new ArrayList<>(bundles.subList(getLoopEndAddress(), bundles.size())));
    }

    private void resolvePostLoopDependencies() {
        bundles.subList(getLoopEndAddress(), bundles.size()).forEach(b -> b.getBundle().stream()
                .filter(i -> i instanceof Consumer).forEach(c -> {
                    if (dependencyMatrix.get(c.getAddress()).getPostLoopDependencies().stream().anyMatch(d -> d.getDestination() == ((Consumer) c).getOperandA())) {
                        ...
                        int dest = producer.getMappedDestination() + calculateLoopStageDiff(bundles.get(getLoopEndAddress()-1).getBundle().get(0), producer);
